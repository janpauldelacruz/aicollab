...
const FORMAT_OPTIONS: {
  ...
  },
  {
    ...
    description: 'Readable transcript with formatting',
    ext: '.md',
  },
  {
    id: 'pdf',
...
function buildJsonExport(session: StoredSession, sessionName: string) {
  ...
  return {
    session: {
      ...
      agents: session.agents.map((a) => ({ id: a.id, name: a.name, role: a.role, model: a.model })),
    },
...
function buildMarkdownExport(session: StoredSession, sessionName: string) {
  const messages = session.messages;
  const agentList = session.agents
    .map((a) => `- **${a.name}** (${a.role}) — \`${a.model}\``)
    .join('\n');
  const lines: string[] = [
...
function buildPdfHtml(session: StoredSession, sessionName: string): string {
  const messages = session.messages;
  const agentRows = session.agents
    .map((a) => `<tr><td>${a.name}</td><td>${a.role}</td><td>${a.model}</td></tr>`)
    .join('');
...
<body>
<h1>${sessionName}</h1>
<div class="meta">Exported: ${new Date().toLocaleString()} &nbsp;|&nbsp; ${messages.length} messages &nbsp;|&nbsp; ${session.agents.length} agents</div>
<h2>Agent Roster</h2>
...
export default function ExportModal({ onClose, session }: ExportModalProps) {
  const sessionName = session.topic;
  ...
  const handleExport = () => {
    ...
    setTimeout(() => {
      try {
        if (selected === 'json') {
          const data = buildJsonExport(session, sessionName);
          downloadBlob(
          ...
          const md = buildMarkdownExport(session, sessionName);
          downloadBlob(md, `${slug}-transcript.md`, 'text/markdown');
        } else if (selected === 'pdf') {
          const html = buildPdfHtml(session, sessionName);
      ...
      <div className="relative bg-card border border-border rounded-xl shadow-2xl w-full max-w-md p-6 space-y-5">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Download the full conversation with agent contributions
            </p>
          </div>
            ...
            {FORMAT_OPTIONS.map((fmt) => (
              ...
              >
                <Icon
                  ...
                  size={18}
                  className={selected === fmt.id ? 'text-primary' : ''}
                />
        ...
        <div className="bg-muted/40 rounded-lg px-4 py-3 flex items-start gap-2.5">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            {selected === 'pdf'
              ? 'PDF export opens a print dialog — use "Save as PDF" in your browser\'s print settings.'
              : `Includes all ${session.messages.length} messages, agent metadata, timestamps, and message types.`}
          </p>
        </div>
