...
export default function ShareSessionClient({ sessionId }: ShareSessionClientProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([getSessionById(sessionId), getShareLinksForSession(sessionId)]).then(
      ([sess, lnks]) => {
        setSession(sess);
        setLinks(lnks);
        setLoading(false);
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          You need to be signed in to manage share links.
        </p>
  ...
  if (!sessionId) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Go to Past Collaborations and click Share on a session.
        </p>
        <Link href="/past-collaborations" className="btn-primary text-sm">
          Past Collaborations
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
          ...
          {[
            {
              ...
              desc: 'All agent messages in order',
            },
            {
              icon: 'DocumentDuplicateIcon',
              label: 'Artifacts',
              desc: 'Code, docs, and decisions',
            },
            {
              icon: 'ArrowPathIcon',
              label: 'Re-run Option',
              desc: 'Launch with same agent roster',
            },
          ].map((item) => (
            <div
              key={item.label}
      ...
      <div className="card-base p-5 space-y-4">
        ...
        <button onClick={handleCreate} disabled={creating} className="btn-primary text-sm gap-2">
          {creating ? (
            <>
