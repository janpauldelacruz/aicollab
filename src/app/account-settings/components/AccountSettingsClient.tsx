...
export default function AccountSettingsClient() {
    ...
    try {
      ...
      if (sessions && sessions.length > 0) {
        const totalArtifacts = sessions.reduce(
          (sum: number, s: any) => sum + (s.artifact_count ?? 0),
          0
        );
  ...
  const statCards = [
    {
      label: 'Sessions Run',
      value: usage.sessionsRun,
      icon: 'PlayCircleIcon',
      color: 'text-primary',
    },
    {
      label: 'Total Artifacts',
      value: usage.totalArtifacts,
      icon: 'FileTextIcon',
                ...
                <div className="bg-card border border-border rounded-xl p-5">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        Pre-fill agent roles when creating new sessions
                      </p>
                    </div>
                    <button
                  ...
                  <div className="space-y-3">
                    {modelPrefs.map((pref, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-3 p-3 rounded-lg bg-background border border-border/60"
                      >
                ...
                <div className="bg-card border border-border rounded-xl p-5">
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      {
                        label: 'Enable real-time artifact preview',
                        key: 'artifactPreview',
                        value: false,
                      },
                    ].map((setting) => (
                      <div key={setting.key} className="flex items-center justify-between py-2">
                        ...
                        >
                          <span
                            className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${setting.value ? 'translate-x-5' : 'translate-x-0.5'}`}
                          />
                        </button>
                      </div>
                    ))}
              ...
              <div className="space-y-5">
                      ...
                      [
                        {
                          ...
                          desc: 'All agent messages and responses',
                        },
                        {
                          key: 'includeArtifacts',
                          label: 'Artifacts',
                          desc: 'Generated code, docs, and files',
                        },
                        {
                          key: 'includeAgentInfo',
                          label: 'Agent Configuration',
                          desc: 'Model names, roles, and settings',
                        },
                        {
                          key: 'includeSummary',
                          label: 'Session Summary',
                          desc: 'Auto-generated overview and key points',
                        },
                ...
                <div className="bg-card border border-border rounded-xl p-5">
                  ...
                  <div className="rounded-lg bg-background border border-border p-3 font-mono text-xs text-muted-foreground space-y-1">
                    ...
                    {exportDefaults.includeAgentInfo && (
                      <p>## Agents: Architect · Developer · Reviewer</p>
                    )}
