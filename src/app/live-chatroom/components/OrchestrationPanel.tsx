...
const ORCHESTRATION_MODES: { value: OrchestrationMode; label: string; icon: string; description: string }[] = [
  { value: 'round-robin', label: 'Round Robin', icon: 'ArrowPathIcon', description: 'Agents take turns in order' },
  { value: 'parallel', label: 'Parallel', icon: 'BoltIcon', description: 'All agents respond simultaneously' },
  { value: 'sequential', label: 'Sequential', icon: 'QueueListIcon', description: 'Each agent completes before next starts' },
  { value: 'priority', label: 'Priority', icon: 'FunnelIcon', description: 'Agents respond by priority rank' },
  { value: 'reactive', label: 'Reactive', icon: 'CpuChipIcon', description: 'Agents respond based on relevance' },
];
...
export default function OrchestrationPanel({
  ...
  const handleInject = () => {
    ...
    onInjectDirective(directiveText.trim(), targetAgentId === '__all__' ? undefined : targetAgentId);
    setDirectiveText('');
      ...
      <div className="flex items-center gap-3 px-4 py-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-muted/40 border border-border">
            ...
            <span className="text-xs font-mono text-muted-foreground tabular-nums">{turnCount}/{maxTurns}</span>
          </div>
        </div>
      </div>
      ...
      {isExpanded && (
        <div className="px-4 pb-4 space-y-4 border-t border-border/50 pt-3">
          ...
          <div>
                ...
                <button
                  ...
                  className={`flex flex-col items-center gap-1 p-2 rounded-lg border text-center transition-all text-xs ${
                    orchestrationMode === mode.value
                      ? 'border-accent/50 bg-accent/10 text-accent' :'border-border bg-muted/20 text-muted-foreground hover:border-border/80 hover:text-foreground disabled:opacity-40 disabled:cursor-not-allowed'
                  }`}
            ...
            {sessionStatus === 'running' && (
              <p className="text-xs text-muted-foreground mt-1.5">⚠ Pause session to change execution mode</p>
            )}
          </div>
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {agents.map((agent, i) => {
                const isActive = orchestrationMode === 'round-robin' && i === activeAgentIndex && sessionStatus === 'running';
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {isActive && <span className="w-1.5 h-1.5 rounded-full bg-positive live-indicator" />}
                        {isPinned && <Icon name="LockClosedIcon" size={10} className="text-warning" />}
                      </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-1">
                {[25, 50, 100].map(v => (
                  <button
                    key={v}
                    onClick={() => { setLocalMaxTurns(v); onMaxTurnsChange(v); }}
