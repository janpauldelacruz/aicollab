    ...
    <div className="flex flex-col h-full bg-card/30">
      ...
      {selectedArtifact ? (
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-3">
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Created by {selectedArtifact.createdBy} at {selectedArtifact.createdAt}
              </p>
            </div>
            <span
              className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-medium border ${TYPE_COLORS[selectedArtifact.type]}`}
            >
          ...
          <div className="flex gap-2">
            <button
              onClick={() => toast.success(`"${selectedArtifact.name}" copied to clipboard`)}
              className="btn-secondary text-xs flex-1 gap-1.5"
            >
              <Icon name="ClipboardDocumentIcon" size={13} />
              Copy
            </button>
            <button
              onClick={() => toast.success(`"${selectedArtifact.name}" downloaded`)}
          ...
          {artifacts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-xs text-muted-foreground/60 mt-1">
                Agents will save code, docs, and decisions here as they work
              </p>
            </div>
          ) : (
            artifacts.map((artifact) => (
              ...
              >
                <div
                  className={`w-7 h-7 rounded-md border flex items-center justify-center flex-shrink-0 ${TYPE_COLORS[artifact.type]}`}
                >
                ...
                <Icon
                  ...
                  size={13}
                  className="text-muted-foreground/40 group-hover:text-primary flex-shrink-0 mt-0.5 transition-colors"
                />
              </button>
            ))
          )}
      ...
      {!selected && artifacts.length > 0 && (
        <div className="p-3 border-t border-border flex-shrink-0">
          <button
            onClick={() => toast.success(`${artifacts.length} artifacts exported as ZIP`)}
