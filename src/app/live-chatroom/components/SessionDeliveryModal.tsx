      ...
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: 'Duration', value: formatDuration(elapsedSeconds), icon: 'ClockIcon', color: 'text-warning' },
              { label: 'Messages', value: String(totalMessages), icon: 'ChatBubbleLeftRightIcon', color: 'text-primary' },
              { label: 'Artifacts', value: String(artifacts.length), icon: 'DocumentDuplicateIcon', color: 'text-accent' },
              { label: 'Completion', value: `${completionPct}%`, icon: 'CheckCircleIcon', color: 'text-positive' },
            ].map((s) => (
          ...
          <div className="rounded-xl border border-border bg-muted/20 p-4 space-y-3">
            ...
            <div className="space-y-2">
              {agents.map((agent) => {
                const pct = totalMessages > 0 ? Math.round((agent.messageCount / totalMessages) * 100) : 0;
                return (
                  <div key={agent.id} className="space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-mono text-muted-foreground">{agent.messageCount} msgs · {pct}%</span>
                    </div>
          ...
          {artifacts.length > 0 && (
            <div className="rounded-xl border border-border bg-muted/20 p-4 space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {artifacts.map((a) => (
                  <div key={a.id} className="flex items-center gap-2.5 p-2.5 rounded-lg border border-border bg-background/40">
                    <Icon name="DocumentTextIcon" size={14} className="text-accent flex-shrink-0" />
                    <div className="min-w-0">
                      <p className="text-xs font-medium text-foreground truncate">{a.name}</p>
                      <p className="text-xs text-muted-foreground capitalize">{a.type}{a.language ? ` · ${a.language}` : ''}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="rounded-xl border border-positive/20 bg-positive/5 p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-foreground mb-1">What was accomplished</h3>
                <p className="text-sm text-muted-foreground leading-relaxed">
                  {agents.length} AI agents collaborated on <span className="text-foreground font-medium">"{topic}"</span> for {formatDuration(elapsedSeconds)}, producing {totalMessages} messages
                  {artifacts.length > 0 ? ` and ${artifacts.length} artifact${artifacts.length !== 1 ? 's' : ''}` : ''}.
                  {completionPct === 100
                    ? ' The session ran to full completion.'
                    : ` The session reached ${completionPct}% of the planned turns.`}
                  {keyDecisions.length > 0 ? ` ${keyDecisions.length} key decision${keyDecisions.length !== 1 ? 's' : ''} were identified.` : ''}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="p-5 border-t border-border flex-shrink-0 flex flex-col sm:flex-row gap-3">
          <button
            ...
            className="btn-secondary text-sm gap-2 flex-1"
          >
