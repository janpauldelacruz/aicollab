...
const MODES: {
...
}[] = [
  {
    value: 'build',
    label: 'Build',
    description: 'Agents take roles and produce real deliverables end-to-end',
  ...
  },
  {
    value: 'brainstorm',
    label: 'Brainstorm',
    description: 'Agents debate and generate creative ideas on a topic',
    icon: 'LightBulbIcon',
  ...
  },
  {
    value: 'code',
    label: 'Code',
    description: 'Agents collaborate to plan, write, and review code',
  ...
  },
  {
    value: 'chat',
    label: 'Open Chat',
    description: 'Agents have free-form conversation on any topic',
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Define the task, mode, and parameters for this collaboration session
        </p>
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-2">
          A short, descriptive name to identify this session
        </p>
        <input
          type="text"
          ...
          {MODES.map((m) => (
            ...
            >
              <Icon
                name={m.icon as any}
                size={20}
                className={config.mode === m.value ? '' : 'text-muted-foreground'}
              />
              <div>
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-2">
          The specific subject or problem the agents will work on
        </p>
        <textarea
          rows={3}
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-2">
          What should the agents have produced or decided by the end?
        </p>
        <textarea
          rows={2}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mb-2">
            Maximum number of agent messages before the session ends
          </p>
          <input
            type="number"
            min={10}
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mb-2">
            Seconds an agent has to respond before being skipped
          </p>
          <input
            type="number"
            min={10}
