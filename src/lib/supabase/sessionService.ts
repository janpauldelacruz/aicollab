...
export async function createSession(config: SessionConfig, userId: string): Promise<DBSession | null> {
...
export async function updateSessionStatus(
  ...
  extra?: Partial<Pick<DBSession, 'turn_count' | 'message_count' | 'artifact_count' | 'elapsed_seconds' | 'completion_pct' | 'started_at' | 'completed_at'>>
): Promise<void> {
  const supabase = createClient();
...
export async function insertMessage(sessionId: string, msg: {
  agentId: string;
  agentName: string;
  agentRole: string;
  agentColor: string;
...
export async function insertArtifact(sessionId: string, artifact: {
  artifactKey: string;
  name: string;
  type: string;
  content: string;
...
export async function getSessionByShareToken(token: string): Promise<{
  ...
  const [agentsRes, messagesRes, artifactsRes] = await Promise.all([
    ...
    supabase.from('session_messages').select('*').eq('session_id', link.session_id).order('created_at', { ascending: true }),
