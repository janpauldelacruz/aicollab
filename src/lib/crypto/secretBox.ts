 ...
 * anything that can run script in the page could exfiltrate every key — which
 * is exactly what the previous base64 "encryption" allowed.
 *
...
export function decryptSecret(stored: string): string {
  ...
  if (parts.length !== 4 || parts[0] !== VERSION) {
    // Rows written by the old build hold bare base64. Reject rather than guess:
    // a silently wrong key produces confusing upstream 401s.
